...
export const listConversion = async (inputsList: InputsList, settings: Set): Promise<[ConvIngr | null, Error | null][]> => {

  let retList: [ConvIngr | null, Error | null][] = []
  ...
  // let ingrs: [IngrInput | null, Error | null][] = rows.map(row => validateList(row))
  let ingrs: [IngrInput | null, Error | null][] = rows.map(row => validateList(row))
  ...
  for (let i = 0; i < ingrs.length; i++) {
    let curr = ingrs[i]
    if (curr[0] !== null) {
      const converted: [ConvIngr | null, Error | null] = await handleConversion(curr[0], settings)
      retList.push(converted)

    } else {
      retList.push([null, curr[1]])
    }
  }
...
const handleConversion = async (inputs: IngrInput, settings: Set): Promise<[ConvIngr | null, Error | null]> => {
  const isAmount: number | null = validateAmount(inputs.currentAmount)
  if (!isAmount) {
    let error: Error = { name: "Amount", message: "Unable to validate amount, either use decimal (1.5) or fraction (1 1/2) amounts" }
    return [null, error]
  }
  ...
  if (isSimple) {
    if (settings.toleranceType === "percent" || unitDict[inputs.currentUnit].normUnit === unitDict[settings.toleranceType].normUnit) {
      ...
      let error: Error = { name: "Tolerance", message: `The tolerance type is set to ${issue}, which does not match what you are trying to convert. Please update tolerance setting to ${changeTo} or percent` }
      return [null, error]
    }
  }
  else {
    if (inputs.ingredientName.length === 0) {
      ...
      let error: Error = { name: "Ingredient Name", message: "An ingredient name is needed for complex conversion" }
      return [null, error]
    }
...
export const formConversion = async (inputs: IngrInput, settings: Set): Promise<[ConvIngr | null, Error | null]> => {
  ...
  return await handleConversion(inputs, settings)
}
...
export const convertSimple = (isAmount: number, inputs: IngrInput, settings: Set): ConvIngr => {
  ...
  const convertedIngr: ConvIngr = {
    currAmtStr: inputs.currentAmount,
...
// type '[import("/home/shannon/projects/unit-conversion/unit-conversion-fe/src/types").IngrInput | null, Error | null][]' is not assignable to
// type '[import("/home/shannon/projects/unit-conversion/unit-conversion-fe/src/types").IngrInput | null, import("/home/shannon/projects/unit-conversion/unit-conversion-fe/src/types").Error | null][]'.